...
## we see datatypes of all the features are numerical in nature
...
## impute missing values with the best method %%%%%% need to check %%%%% for now using median imputation 
absenteeism_data = impute_median_mode(absenteeism_data)
...
##################### Sampling ######################################################################
## Let us separate our data set into test and train 
...
#We chose 75% of the data in he train set and 25% in the test set
train = createDataPartition(absenteeism_data$Reason.for.absence,times = 1,p = 0.75,list = F)
test = -(train)
...
summary(modelLR)
#modelLR = lm(Absenteeism.time.in.hours~Reason.for.absence+Distance.from.Residence.to.Work+Son,data = absenteeism_data[train,])
# summary(modelLR)
